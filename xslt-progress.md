...
Implemented the XTDE0410 dynamic error check per XSLT 3.0 §5.7.1: "It is a
dynamic error if the result sequence used to construct the content of an element
node contains an attribute node following a node that is not an attribute or
namespace node."

Changes:
- Added `XTDE0410` error variant to `xee-interpreter/src/error.rs`
- Added tracking in `xml_append_items()` to detect when attribute/namespace nodes
  follow non-attribute/namespace children, both from pre-existing children and
  within the current append sequence
- Handles edge case of array recursion re-deriving child state

Conformance: 5993 → 5998 passed (+5), 2005 → 2000 filtered (-5), 0 failed.
Tests fixed: attribute-0803, attribute-0804, attribute-0805, copy-4701, copy-4702,
error-0410a (already unfiltered).
